...
public class AppLogistik {
	 public static ArrayList<String> pesanan = new ArrayList<String>();
	    ...
	    public static final String angka = "[0-9]+";
	    public static boolean is_exit = false;
	    
	    public static void main(String[] args) {
	        String[] menus = {"Input Pesanan", "Lacak Pesanan"};
	        String pilih_menus = "Pilih menu di bawah ini:\n";

	        for(int i = 0; i < menus.length; i++) {
	            pilih_menus += (i + 1) + ". " + menus[i] + "\n";
	        }

	        while(!is_exit) {
	            String pilihan_menu = JOptionPane.showInputDialog(null, pilih_menus, "Aplikasi PT. XYZ", JOptionPane.QUESTION_MESSAGE);
	            
	            if((pilihan_menu != null) && (pilihan_menu.matches(angka))) {
	            ...
	            } else if((pilihan_menu != null) && (!pilihan_menu.matches(angka))) {
	    ...
	    private void InputPesanan() {
	        final String[] satuan_pesanan = {"Berat Barang (gr)", "Jarak Barang (km)"};
	        boolean is_exit_input_pesanan = false;
	        int i = 0;

	        while(!is_exit_input_pesanan) {
	            while(i < satuan_pesanan.length) {
	                String values = JOptionPane.showInputDialog(null,
	                                "Masukkan " + satuan_pesanan[i] + ":",
	                                "Input " +satuan_pesanan[i],
	                                JOptionPane.INFORMATION_MESSAGE);

	                if((values != null) && (values.matches(angka))) {
	                    pesanan.add(values);
	                    i++;
	                    if(i == 2) { mainApps.CetakResi(); }
	                } else if((values != null) && (!values.matches(angka))) {
	                    JOptionPane.showMessageDialog(null, "Inputan harus angka!", "PERINGATAN", JOptionPane.ERROR_MESSAGE);
	                    i = 0;
	                } else {
